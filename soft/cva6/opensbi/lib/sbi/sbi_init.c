...
static void __noreturn init_coldboot(struct sbi_scratch *scratch, u32 hartid)
{
	...
	/**************************** */
	// debug: print final message before jump
	/* Comprobar que el DTB parece un DTB */
	volatile u32 *fdt_magic_ptr =
		(u32 *)(uintptr_t)scratch->next_arg1;
	volatile u64 *kptr =
		(u64 *)(uintptr_t)scratch->next_addr;

	sbi_printf("[CVA6][DEBUG] next_addr = 0x%lx\n",
		   scratch->next_addr);
	sbi_printf("[CVA6][DEBUG] next_arg1 (FDT) = 0x%lx\n",
		   scratch->next_arg1);

	/* OJO: si aquí el bus se cuelga, no llegarás a ver el print de magic */
	u32 magic = *fdt_magic_ptr;
	sbi_printf("[CVA6][DEBUG] FDT magic = 0x%08x (esperado 0xd00dfeed)\n",
		   magic);

	/* Leer las dos primeras palabras del kernel */
	u64 w0 = kptr[0];
	u64 w1 = kptr[1];
	sbi_printf("[CVA6][DEBUG] first kernel words = 0x%016lx 0x%016lx\n",
